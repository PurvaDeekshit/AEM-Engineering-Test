...
public class IntegerToRoman {
    ...
    //check if min number is less than max number
    public Boolean validateMinLessThanMax(Integer min , Integer max) {
        if (min >= max) {
            logger.error("Min number " + min + "is greater than or equal to max number " + max);
            return false;
        }
        return true;
    }
